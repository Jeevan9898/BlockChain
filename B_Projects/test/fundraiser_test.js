...
contract("Fundraiser", accounts => {
    ...
    describe("setBeneficiary", () => {
        ...
        it("updates beneficiary when called by owner account", async () => {
            ...
            const actual = await fundraiser.beneficiary();
            assert.equal(actual, newBeneficiary);
        });

       it("throws an error when called from a non-owner account", async () => {
            try {
                await fundraiser.withdraw({ from: accounts[3] });
                assert.fail("should not be permitted");
            } catch (err) {
                assert(
                    err.message.includes("revert"),
                    "Expected revert but got something else"
                );
            }
        });
    });
    ...
    describe("making donations", ()=>{
        ...
        it("includes donation in myDonations", async()=>{
            ...
            assert(dates[0], "dates should be present");
        });
        ...
        it("increases donationsCount", async()=>{
            ...
            await fundraiser.donate({from : owner, value});
